...
from oclaw.openclaw_runtime.skills import discover_workspace_skill_manifests, load_skill_manifest
...
def _skills_bins_handler(opts: dict[str, Any]) -> None:
    ...
    for m in discover_workspace_skill_manifests():
        oc = dict(m.metadata_openclaw or {})
        req = oc.get("requires")
...
def _skills_detail_handler(opts: dict[str, Any]) -> None:
    ...
    _ok(
        respond,
        {
            ...
            "metadata": {"openclaw": dict(manifest.metadata_openclaw)},
            "body": manifest.body,
        },
    )
...
def _skills_install_handler(opts: dict[str, Any]) -> None:
    ...
    md = params.get("metadata_openclaw")
