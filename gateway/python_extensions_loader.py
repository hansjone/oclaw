...
from oclaw.extensions.plugin_api import PluginEntry
    ...
    *,
    ...
    for ext_dir in _iter_extension_dirs(root):
        ...
        if wanted and ext_id not in wanted:
            continue
        module_name = f"openclaw_pyext_{_sanitize_module_name(ext_id)}"
...
def build_python_extensions_registry(
    *,
    ...
    # - repo root (contains `oclaw/extensions`)
    # - `oclaw` package root (contains `extensions`)
    extension_roots = [ws_dir / "oclaw" / "extensions", ws_dir / "extensions"]
