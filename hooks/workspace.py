...
from .frontmatter import parse_frontmatter, resolve_openclaw_metadata
...
def _load_hook_from_dir(hook_dir: Path, source: HookSource, plugin_id: Optional[str] = None) -> Optional[HookEntry]:
    ...
    metadata = resolve_openclaw_metadata(fm) or {}
...
def discover_workspace_hook_entries(
    ...
    *,
    ...
    """
    Python port of OpenClaw hook discovery strategy.

    - bundled: shipped with runtime
    - managed: user config dir (~/.openclaw/hooks by default)
    - workspace: <workspace>/hooks (explicit opt-in by default)
    - extra: any extra managed dirs
    """
    ws = Path(workspace_dir)
    managed = Path(managed_hooks_dir) if managed_hooks_dir else (Path.home() / ".openclaw" / "hooks")
    ...
    if extra_dirs:
        for raw in extra_dirs:
            ...
            entries.extend(load_hook_entries_from_dir(str(p), source="openclaw-managed"))

    if bundled_hooks_dir:
        entries.extend(load_hook_entries_from_dir(bundled_hooks_dir, source="openclaw-bundled"))

    entries.extend(load_hook_entries_from_dir(str(managed), source="openclaw-managed"))
    entries.extend(load_hook_entries_from_dir(str(workspace_hooks), source="openclaw-workspace"))
    return entries
