...
from oclaw.platform.persistence.sqlite_store import OpenClawTask, SqliteStore
...
def _payload(task: OpenClawTask) -> dict[str, Any]:
    try:
...
def _capture_after_turn(*, wiki_root: Path, task: OpenClawTask, payload: dict[str, Any]) -> dict[str, Any]:
...
def _process_task(*, task: OpenClawTask, handlers: dict[str, Any], plugin_cfg: dict[str, Any]) -> dict[str, Any]:
    wiki_root = _resolve_wiki_root(plugin_cfg)
...
def run_worker() -> int:
    ...
    while True:
        ...
        if not {"wiki_apply", "wiki_lint"}.issubset(set(handlers.keys())):
            time.sleep(interval_s)
            continue
        task = store.openclaw_task_claim(worker_id=worker_id, lease_seconds=120, task_type="wiki_capture")
        if task is None:
            time.sleep(interval_s)
            continue
        try:
            result = _process_task(task=task, handlers=handlers, plugin_cfg=plugin_cfg)
            store.openclaw_task_finish(task_id=task.id, result=result)
        except Exception as exc:
            store.openclaw_task_fail(task_id=task.id, error=f"{type(exc).__name__}: {exc}", result={"ok": False})
            time.sleep(interval_s)
