...
1. **生成工具不会自动发送附件**  
   `write_file`、`run_command`、`cloudflare_image_generate` 等只产生内容，渠道出站看不到。
...
|----------|--------------------------------------|
| workspace 文件（csv/txt/xlsx） | `path="data/workspace/..."` |
| 生图/生视频（已有 attachment_id） | `attachment_id="..."` |
...
| `.xlsx`、`.xls` 等 Excel | `run_command` | 用 Python（`openpyxl` 已安装） |
| 图片 | `cloudflare_image_generate` 等 | 生成后用 `attachment_id` 标记 |
| 视频 | 对应生成工具 | 生成后用 `attachment_id` 标记 |

**不要声称没有 `run_command`。** 用户要真 `.xlsx` 时优先用 `run_command`，不要只写 CSV 代替。
