...
function md5Local(str) {
  ...
  function md51(s) {
    ...
    for (let i = 64; i <= s.length; i += 64) md5cycle(state, md5blk(s.substring(i - 64, i)))
    s = s.substring(i - 64)
...
// DSH Cordis Client 插件
export const name = 'uds-auth-ui'
export const inject = ['slots']

export function apply(ctx) {
  const slots = ctx.slots
  if (!slots) {
    console.warn('[uds-auth-ui] slots not available')
    return
  }

  // 注入 CSS
  if (!document.getElementById('uds-auth-cordis-css')) {
    const style = document.createElement('style')
    style.id = 'uds-auth-cordis-css'
  ...
  function updateBadge(user) {
    ...
    if (!user) {
      ...
      const name = user.userName || user.name || '用户' + user.empNo
      const initials = name.slice(0, 2).toUpperCase()
      const roleLabel = getRoleLabel(user.role)
      const roleBadge = roleLabel ? '<span class="uds-auth-role-badge uds-auth-role-' + user.role + '">' + roleLabel + '</span>' : ''
      badge.innerHTML = '<span class="uds-auth-avatar">' + initials + '</span><span>' + name + '</span>' + roleBadge
  ...
  function toggleDropdown() {
    ...
    const el = document.getElementById('uds-auth-dropdown-el')
    if (el) el.style.display = dropdownVisible ? 'block' : 'none'
    ...
    if (!currentUser) {
      container.innerHTML = `
        ...
        <div class="uds-auth-qr-container">
          <div id="uds-qr-wrap" style="text-align:center"></div>
      ...
      const sBtn = document.getElementById('uds-settings-btn')
      if (sBtn) sBtn.onclick = () => { window.location.href = '/settings' }
      const uBtn = document.getElementById('uds-user-mgmt-btn')
      if (uBtn) uBtn.onclick = openUserManagement
      const lBtn = document.getElementById('uds-logout-btn')
  ...
  function startQRLogin() {
    ...
    const wrap = document.getElementById('uds-qr-wrap')
    if (!status) return
    ...
    loadUACMd5Plugin().then(() => {
      ...
      if (wrap) wrap.innerHTML = '<img class="uds-auth-qr-img" src="/uds-auth/qr?data=' + encodeURIComponent(qrCodeStr) + '">'
      qrPollingTimer = setInterval(pollQRStatus, 2000)
    }).catch(() => { status.textContent = '加载加密插件失败' })
  }
  ...
  function pollQRStatus() {
    ...
    const verifyCode = window.jQuery ? window.jQuery.verfiyCode(source) : md5Local(source)
    const body = JSON.stringify({
      qrCodeKey: currentQRKey, qrCodeValue: currentQRValue,
      loginClientIp: '127.0.0.1', originSystemCode: '',
      loginSystemCode: LOGIN_SYSTEM_CODE, verifyCode
    })
    fetch('/uds-auth/qr-proxy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body })
      .then(r => r.json()).then(handleQRResult)
      .catch(() => {
  ...
  function handleQRResult(result) {
    const code = result.code || {}, bo = result.bo || {}
    const codeCode = code.code || '', boCode = bo.code || ''
    const status = document.getElementById('uds-qr-status')
    ...
    if (codeCode === '0000' && boCode === '0000') {
      ...
      if (empNo) {
        ...
        setTimeout(() => { closeDropdown(); fetchUserAndPermissions() }, 500)
      }
      return
    }
    if (codeCode === '0000' && boCode === '4002') { if (status) status.textContent = '等待扫码...'; return }
    ...
    fetch(url)
      ...
      .then(result => {
        if (result.code && result.code.code === '0000' && result.bo && result.bo.length > 0) {
          ...
          const dept = emp.deptName || emp.deptShortName || emp.orgName || emp.department || ''
          const name = emp.name || emp.empName || emp.userName || '用户' + empNo
          cb({ empNo: emp.employeeShortId || emp.employeeNO || empNo, userName: name, department: dept, email: emp.email || emp.mail || '', phone: emp.mobile || emp.phone || '' })
        } else cb({ empNo, userName: '用户' + empNo })
      })
      ...
      .then(data => {
        ...
        users.forEach(u => {
          ...
          html += '<tr><td>' + u.empNo + '</td><td>' + u.role + '</td><td>' +
            '<select onchange="window.__udsChgRole(\'' + u.empNo + '\',this.value)">' + roleOpts + '</select> ' +
            '<button onclick="window.__udsDelUser(\'' + u.empNo + '\')" class="uds-auth-del-btn">删除</button></td></tr>'
        })
        html += '</tbody></table>'
        html += '<div class="uds-auth-add-user"><input id="uds-new-empno" placeholder="工号"><select id="uds-new-role"><option value="user">普通用户</option><option value="admin">管理员</option></select><button onclick="window.__udsAddUser()">添加</button></div></div>'
  ...
  window.__udsChgRole = (empNo, role) => {
  ...
  // 注册 React 组件 - 徽章按钮
  function BadgeButton({ isAuth, name, initials, roleBadge }) {
    return React.createElement('button', {
      id: 'uds-auth-badge-el',
      className: 'uds-auth-badge ' + (isAuth ? '' : 'uds-auth-badge-unauth'),
      ...
      style: {
        ...
        color: isAuth ? 'var(--dsw-alias-label-primary,#1f2329)' : 'var(--dsw-alias-label-tertiary,#8f959e)',
        fontSize: '13px', cursor: 'pointer', fontWeight: '500'
      }
    },
      React.createElement('span', {
        className: 'uds-auth-avatar',
        style: {
          width: '22px', height: '22px', borderRadius: '50%',
          background: isAuth
            ? 'linear-gradient(135deg,var(--dsw-alias-state-business-primary,#3370ff),var(--dsw-alias-state-success-primary,#20a162))'
            : 'var(--dsw-alias-bg-module-platform,rgba(242,243,245,1))',
          color: isAuth ? '#fff' : 'var(--dsw-alias-label-tertiary,#8f959e)',
  ...
  // 注册到 sidebar.footer.action slot (scope: root, 所有页面都显示)
  slots.inject('sidebar.footer.action', () => {
    slots.register(
      { name: 'sidebar.footer.action', id: 'uds-auth-login', order: 100 },
      (props) => {
        const isAuth = !!currentUser
        const name = currentUser ? (currentUser.userName || currentUser.name || '用户' + currentUser.empNo) : '未登录'
        const initials = name.slice(0, 2).toUpperCase()
        const roleLabel = currentUser ? getRoleLabel(currentUser.role) : ''
        const roleBadge = roleLabel
          ? React.createElement('span', { className: 'uds-auth-role-badge uds-auth-role-' + currentUser.role }, roleLabel)
          : null

        return React.createElement('div', { style: { position: 'relative', display: 'flex', alignItems: 'center' } },
          BadgeButton({ isAuth, name, initials, roleBadge }),
          React.createElement('div', {
            id: 'uds-auth-dropdown-el',
            className: 'uds-auth-dropdown',
            style: {
              display: 'none', position: 'absolute', bottom: '100%', left: '0', marginBottom: '8px',
  ...
  document.addEventListener('click', (e) => {
    ...
    if (dropdownVisible && badge && !badge.contains(e.target) && dropdown && !dropdown.contains(e.target)) {
      closeDropdown()
    }
  })
