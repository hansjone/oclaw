 ...
 * 登录徽章 - 注册到 sidebar.footer.action (scope: root, 所有页面都显示)
 */
const CSS = `
...
function getCookie(name) {
  var c=name+'=',arr=document.cookie.split(';')
  for(var i=0;i<arr.length;i++){var v=arr[i].trim();if(v.indexOf(c)===0)return decodeURIComponent(v.substring(c.length))}
  return null
}
...
module.exports = {
  ...
  apply(ctx) {
    ...
    if (!slots) { console.warn('[uds-auth-ui] slots not available'); return }
    ...
    slots.inject('sidebar.footer.action', () => {
      slots.register({ name: 'sidebar.footer.action', id: 'uds-auth-login', order: 100 }, (props) => {
        ...
        const roleBadge = roleLabel ? h('span', {className:'uds-auth-role-badge uds-auth-role-'+currentUser.role}, roleLabel) : null
        ...
        return h('div', {style:{position:'relative',display:'flex',alignItems:'center'}},
          h('button', {
            ...
            h('span', {className:'uds-auth-avatar',style:{width:'22px',height:'22px',borderRadius:'50%',
              background: isAuth ? 'linear-gradient(135deg,var(--dsw-alias-state-business-primary,#3370ff),var(--dsw-alias-state-success-primary,#20a162))' : 'var(--dsw-alias-bg-module-platform,rgba(242,243,245,1))',
              color: isAuth ? '#fff' : 'var(--dsw-alias-label-tertiary,#8f959e)',
              display:'flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:'700'}}, initials),
            h('span', null, name),
            roleBadge
          ),
          h('div', {
