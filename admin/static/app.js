...
async function renderPlugins() {
  ...
  let toolPolicy = {
    ...
    mcp_env_allowlist: "",
    openclaw_retryable_error_codes: "",
  ...
  const openclawRetryableErrorCodesInput = el("input", {
    class: "input",
    value: String(toolPolicy.openclaw_retryable_error_codes || ""),
    placeholder: "provider_timeout,provider_rate_limited,...",
  });
  const openclawRetryCodesStrictModeCb = el("input", { type: "checkbox" });
  openclawRetryCodesStrictModeCb.checked = !!toolPolicy.openclaw_retry_codes_strict_mode;
  ...
  const saveToolPolicyBtn = el("button", {
    ...
    onclick: async () => {
      const r = await apiPost("/admin/api/tool-policy", {
        ...
        openclaw_retryable_error_codes: String(openclawRetryableErrorCodesInput.value || ""),
        openclaw_retry_codes_strict_mode: !!openclawRetryCodesStrictModeCb.checked,
  ...
  const foldToolPolicy = pluginsFold(`【1】工具策略与已注册插件（${pluginCatalog.length}）`, [
    ...
    el("div", { class: "row" }, [
      ...
      openclawRetryableErrorCodesInput,
    ]),
    el("label", { class: "kv" }, [openclawRetryCodesStrictModeCb, document.createTextNode(" Strict mode: reject unknown retry codes")]),
    el("div", { class: "row" }, [
