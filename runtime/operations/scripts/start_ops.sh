...
if [[ "$SKIP_CONFIG_HINT" -eq 0 ]]; then
  ...
  python -m oclaw.runtime.operations channel wecom status || true
  cat <<'EOF'

If WeCom is not configured yet, run:
  python -m oclaw.runtime.operations channel wecom config --help
EOF
fi

step "Stopping previous stack"
python -m oclaw.runtime.operations stack down || true

step "Starting stack"
python -m oclaw.runtime.operations stack up --channel "$CHANNEL"
