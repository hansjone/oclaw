...
if (-not $SkipConfigHint) {
    ...
    Write-Host "  python -m oclaw.ops channel wecom config --help"
    Write-Host ""
}

Write-Step "Stopping previous stack (safe if not running)"
& $pythonExe -m oclaw.ops stack down

Write-Step "Starting stack: python -m oclaw.ops stack up --channel $Channel"
& $pythonExe -m oclaw.ops stack up --channel $Channel
...
Write-Host "  python -m oclaw.ops stack status"
Write-Host "  python -m oclaw.ops stack down"
