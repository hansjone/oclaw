...
if (Test-Path $configPath) {
    ...
    Write-Step "Using OPENCLAW_CONFIG_PATH: $configPath"
}

if ($Background) {
    Write-Step "Starting gateway in background"
    $p = Start-Process -FilePath $pythonExe -ArgumentList @("-m","oclaw.ops","gateway","start","--host",$BindHost,"--port",$Port) -WorkingDirectory $repoRoot -PassThru -WindowStyle Hidden
...
& $pythonExe -m oclaw.ops gateway start --host $BindHost --port $Port
