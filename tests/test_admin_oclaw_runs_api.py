...
class AdminOclawRunsApiTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.store.oclaw_attempt_append(
            run_id="run-1",
            tenant_id=self.tenant_id,
            session_id=self.session_id,
            attempt_no=1,
    ...
    def test_runs_list_and_get(self) -> None:
        ...
        r1 = self.client.get(f"/admin/api/oclaw/runs?session_id={self.session_id}&include_attempts=1", headers=headers)
        ...
        r2 = self.client.get("/admin/api/oclaw/runs?run_id=run-1&include_attempts=1", headers=headers)
