...
def test_agent_core_run_started_trace_payload_keys(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    run_agent_core(
        ...
        data=AgentCoreRunInput(
            ...
            parent_span_id=None,
            max_attempts=1,
            oclaw_task_id="task-99",
            oclaw_worker_id="worker-zz",
        ),
    )
    ...
    assert pl.get("pipeline") == "oclaw_agent_core"
    assert pl.get("oc_stage") == "run_start"
    assert pl.get("trace_id") == trace_id
    assert pl.get("lang") == "zh"
    assert "run_id" in pl
    assert pl.get("oclaw_task_id") == "task-99"
    assert pl.get("oclaw_worker_id") == "worker-zz"
