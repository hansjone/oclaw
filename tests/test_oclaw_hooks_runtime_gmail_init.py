...
from runtime.hooks_runtime import _reset_hooks_runtime_state_for_test, initialize_hooks_runtime
...
def test_initialize_hooks_runtime_invokes_gmail_lifecycle(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "runtime.hooks.gmail_watcher_lifecycle.start_gmail_watcher_with_logs",
        _cap,
    )
    bundled = tmp_path / "bundled"
    bundled.mkdir()
    monkeypatch.setattr("runtime.hooks_runtime.runtime_hooks_bundled_root", lambda: str(bundled))
