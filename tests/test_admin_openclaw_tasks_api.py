...
class AdminOpenClawTasksApiTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.task_a = self.store.openclaw_task_create(
            tenant_id=self.tenant_id,
        ...
        self.store.openclaw_task_create(tenant_id=other_tenant, session_id=other_session, payload={"text": "secret"})
    ...
    def test_openclaw_tasks_api_lists_only_current_tenant(self) -> None:
        token = self._login()
        headers = {"authorization": f"Bearer {token}"}
        r = self.client.get("/admin/api/openclaw/tasks?limit=50", headers=headers)
    ...
    def test_openclaw_task_get_by_id(self) -> None:
        token = self._login()
        headers = {"authorization": f"Bearer {token}"}
        r = self.client.get(f"/admin/api/openclaw/tasks?task_id={self.task_a.id}", headers=headers)
        self.assertEqual(r.status_code, 200)
        body = r.json()
        self.assertTrue(body.get("ok"))
