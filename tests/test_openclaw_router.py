...
from oclaw.openclaw_runtime.types import StandardMessage, normalize_interaction_mode, normalize_requested_specialist
...
def test_router_llm_json_mode_uses_model() -> None:
    class Store:
        def get_setting(self, key: str) -> str:
            if key == "AIA_OPENCLAW_ROUTER_MODE":
                return "llm_json"
            return ""
...
def test_requested_specialist_normalization_defaults_to_generalist() -> None:
    ...
    assert normalize_requested_specialist("memory_curator") == "memory_curator"
    assert normalize_requested_specialist("memory-curator") == "memory_curator"
