...
from oclaw.agents.agent_scope import resolve_agent_id_by_workspace_path
from oclaw.openclaw_runtime.hooks_runtime import get_active_hooks_config, trigger_hook_event
...
def _project_context_roots() -> tuple[Path, ...]:
    roots: list[Path] = []
    raw_ws = str(os.getenv("OPENCLAW_WORKSPACE") or "").strip()
    if raw_ws:
        roots.append(Path(raw_ws).expanduser())
    roots.append(Path(PROJECT_ROOT) / "oclaw" / "agent" / "workspace-main")
