...
def _ensure_openclaw_path() -> Path:
    openclaw_dir = (Path(PROJECT_ROOT) / "oclaw").resolve()
...
def resolve_runtime_config() -> dict[str, Any]:
    ...
    env_json = str(os.getenv("OPENCLAW_RUNTIME_CONFIG_JSON") or "").strip()
    if env_json:
        try:
    ...
    raw_path = str(os.getenv("OPENCLAW_CONFIG_PATH") or "").strip()
    if raw_path:
...
def initialize_hooks_runtime(*, cfg: dict[str, Any] | None, workspace_dir: str) -> int:
    ...
    try:
        openclaw_dir = _ensure_openclaw_path()
        import hooks as hooks_mod  # type: ignore

        bundled_dir = openclaw_dir / "hooks" / "bundled"
