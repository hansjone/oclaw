...
from oclaw.openclaw_runtime.agent_core_attempt import AttemptRunnerInput, run_attempt
from oclaw.openclaw_runtime.memory_stage import compact_memory_context
from oclaw.openclaw_runtime.types import AttemptState, OpenClawMemoryContext, RunState, StandardMessage
...
class AgentCoreRunInput:
    ...
    openclaw_task_id: str | None = None
    openclaw_worker_id: str | None = None
...
def resolve_retryable_error_codes(*, store: Any) -> set[str]:
    raw = ""
    try:
        raw = str(store.get_setting("AIA_OPENCLAW_RETRYABLE_ERROR_CODES") or "").strip()
    except Exception:
        raw = ""
    if not raw:
...
def _trace(
    ...
    *,
    ...
    merged.setdefault("pipeline", "openclaw_agent_core")
    ...
    tid = str(data.openclaw_task_id or "").strip()
    if tid:
        merged.setdefault("openclaw_task_id", tid)
    wid = str(data.openclaw_worker_id or "").strip()
    if wid:
        merged.setdefault("openclaw_worker_id", wid)
    try:
