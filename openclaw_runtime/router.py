...
from oclaw.openclaw_runtime.types import normalize_interaction_mode, normalize_requested_specialist
...
def _router_mode_from_store(store: Any | None) -> str:
    ...
    if store is not None:
        try:
            raw = str(store.get_setting("AIA_OPENCLAW_ROUTER_MODE") or "").strip().lower()
        except Exception:
            raw = ""
    if not raw:
        raw = str(os.getenv("AIA_OPENCLAW_ROUTER_MODE", "") or "").strip().lower()
    return raw or "rule"
